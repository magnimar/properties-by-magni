...
class RapydService:
    ...
    def _get_signature(http_method, path, salt, timestamp, body):
        ...
        hash_object = hmac.new(
            RAPYD_SECRET_KEY.encode("utf-8"), sig_string.encode("utf-8"), hashlib.sha256
        )

        signature = base64.b64encode(hash_object.hexdigest().encode("utf-8")).decode(
            "utf-8"
        )
        return signature
    ...
    def create_customer(cls, email, name=None):
        path = "/v1/customers"
        body = {"email": email, "name": name or email}
