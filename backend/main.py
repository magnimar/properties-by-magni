...
async def get_my_subscriptions(
    ...
    for sub in subscriptions:
        if sub.get("status") in ("active", "trialing"):
            ...
            if pm_id:
                pm_res = RapydService.get_payment_method(current_user.rapyd_customer_id, pm_id)
                if pm_res.get("status", {}).get("status") == "SUCCESS":
                    pm_data = pm_res.get("data", {})
                    sub_info["card_details"] = {
                        "brand": pm_data.get("image", "").split('/')[-1].split('.')[0].capitalize() or "Card",
...
async def cancel_my_subscription(
    ...
    subs_res = RapydService.list_subscriptions_for_customer(current_user.rapyd_customer_id)
    if subs_res.get("status", {}).get("status") != "SUCCESS":
        raise HTTPException(status_code=502, detail="Failed to verify subscription ownership")
    ...
    cancel_res = RapydService.cancel_subscription(subscription_id, cancel_at_period_end=True)
...
async def reactivate_my_subscription(
    ...
    update_res = RapydService.update_subscription(subscription_id, cancel_at_period_end=False)
...
async def get_my_payment_methods(
    ...
    for pm in pms:
        formatted_pms.append({
            "id": pm.get("id"),
            "brand": pm.get("image", "").split('/')[-1].split('.')[0].capitalize() if pm.get("image") else "Card",
            "last4": pm.get("last4"),
...
async def change_subscription_card(
    ...
    update_res = RapydService.update_subscription(subscription_id, payment_method=req.payment_method_id)
    if update_res.get("status", {}).get("status") != "SUCCESS":
        raise HTTPException(status_code=502, detail="Failed to update subscription payment method")
...
async def sync_subscription_payment(
    ...
    latest_pm = sorted(payment_methods, key=lambda x: x.get('created_at', 0), reverse=True)[0]
    ...
    for sub in subscriptions:
        if sub.get("status") in ("active", "trialing"):
            ...
            update_res = RapydService.update_subscription(sub["id"], payment_method=latest_pm_id)
            if update_res.get("status", {}).get("status") == "SUCCESS":
                updated_count += 1

    return {"message": f"Updated {updated_count} subscription(s) with new card", "card_last4": latest_pm.get("last4")}
